...
for i = sample_indices
    ...
    plot(time(in_range)*1e3, y(in_range), 'bo-');   
    plot(NaN, NaN, 'ro-'); 
    if i > 1
        plot(time(out_range)*1e3, y(out_range), 'r-'); 
        TT=time(out_range);
        YY=y(out_range);
        scatter(TT(2:end)*1e3, YY(2:end), '*','r'); 
    end
end
...
sample_indices_2 = [11 19];  % 指定样本编号

for i = sample_indices_2
    y = crack_data{i};                
    N = length(y);                    
    time = 0:10:(N-1)*10;             
    plot(time*1e3, y, 'kd-');   
...
legend('Data for model establishment', 'Data for degradation prediction', 'Data for RUL prediction', ...
